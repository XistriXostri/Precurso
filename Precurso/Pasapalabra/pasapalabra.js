const catalogue = [
  ...
  const display = document.querySelector(".definition-display");
  const textBox = document.getElementById("text-box");
  const gamepadButtons = document.querySelectorAll("button");
  const gamepadButtonsArray = Array.from(gamepadButtons);
  const answer = "";
  const actualScore = document.querySelector(".actual-score");
  const letterCircle = document.querySelectorAll("p");

  let turn = 0;
  let questions = [];

//Creamos el catalogo para esta partida:
...
gamepadButtonsArray.forEach((button) => {
    button.addEventListener("click", () => {
        pasapalabra(button)
    })
})

const pasapalabra = (button) => {
...
const checkAnswer = () => {
    if (answer.toLowerCase === questions[turn].answer){
        return true;
    }
    return false;
}
...
const round = () => {
    display.innerHTML = questions[turn].question;
}
